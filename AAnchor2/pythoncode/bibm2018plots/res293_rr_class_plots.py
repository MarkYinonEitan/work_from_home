...
def plot_total_accuracy3(acc4, lgnds, lbs_names,lbs_order,title,filename):
...
#    h3, = plt.plot(x_ticks_pos,acc_to_plot[3],'C0-*',label = lgnds[2])
    ...
    plt.grid(True)
    plt.title(title)
    plt.legend(handles=[h0,h1,h2],loc=3 )
    plt.savefig(filename)
...
RES1 = SingleNetResults(NETS_FOLDER+'/v4_db0023_real_rot10_plus_sim/', LabelbyAAType.get_labels_to_names_dict())
RES1.load_data()

RES2 = SingleNetResults(NETS_FOLDER+'/v4_db2729_real_rot10/', LabelbyAAType.get_labels_to_names_dict())
RES2.load_data()


RES3 = SingleNetResults(NETS_FOLDER+'/v4_db2931_real_rot10/', LabelbyAAType.get_labels_to_names_dict())
RES3.load_data()
...
plot_total_accuracy3([RES1.det_acc_per_epoch[-1],RES2.det_acc_per_epoch[-1],RES3.det_acc_per_epoch[-1]],['Res $2.2 \\AA$','Res $2.9 \\AA$','Res $3.1 \\AA$'], RES1.labels_names,labels_to_show,'Train: Real , Test Real',RES_FOLDER+'r3_293_acc.png')
