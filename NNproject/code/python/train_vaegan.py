...
with tf.Session(config=config) as sess:
    ...
    for batch in range(1000000):
            ...
            # optimizaiton G
            sess.run(nn.opti_G, feed_dict=fd)
            # optimization D
            ...
            if batch %10 == 0:
                ...
                D_loss, LL_loss, G_loss, reconstr_loss= sess.run([nn.D_loss, nn.LL_loss, nn.G_loss, nn.reconstr_loss],feed_dict=fd)
                print(" D_loss {} LL_loss {} G_loss {} reconstr_loss {} ".format(D_loss, LL_loss, G_loss, reconstr_loss))
                #Summary
                summary_str = sess.run(summary_op, feed_dict=fd)
                summary_writer.add_summary(summary_str, batch)
            if batch %100 == 0:
                fd ={nn.vx_real: vx_real, nn.mp_real: mp_real}
                loss_vae= sess.run(nn.G_loss,feed_dict=fd)
