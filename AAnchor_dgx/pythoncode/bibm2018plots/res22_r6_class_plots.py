...
def plot_total_accuracy4(acc4, lgnds, lbs_names,lbs_order,title,filename):

    acc_to_plot =[]
    for acc in acc4:
        acc_to_plot.append([acc[x] for x in lbs_order])
    ...
    plt.close()
    h0, = plt.plot(x_ticks_pos,acc_to_plot[0],'C2-o',label = lgnds[0])
    h1, = plt.plot(x_ticks_pos,acc_to_plot[1],'C1-^',label = lgnds[1])
    h2, = plt.plot(x_ticks_pos,acc_to_plot[2],'C3-s',label = lgnds[2])
    h3, = plt.plot(x_ticks_pos,acc_to_plot[3],'C0-*',label = lgnds[3])
    ...
    plt.grid(True)
    plt.title(title)
    plt.legend(handles=[h0,h1,h2,h3],loc=3 )
    plt.savefig(filename)
...
RES1 = SingleNetResults(NETS_FOLDER+'/v4_db0023_real_rot10/', LabelbyAAType.get_labels_to_names_dict())
RES1.load_data()

RES2 = SingleNetResults(NETS_FOLDER+'/v4_db0023_10_nets/nets_1_to_3/', LabelbyAAType.get_labels_to_names_dict())
RES2.load_data()


RES3 = SingleNetResults(NETS_FOLDER+'/v4_db0023_10_nets/nets_1_to_4/', LabelbyAAType.get_labels_to_names_dict())
RES3.load_data()

RES4 = SingleNetResults(NETS_FOLDER+'/v4_db0023_10_nets/nets_1_to_6/', LabelbyAAType.get_labels_to_names_dict())
RES4.load_data()
...
plot_total_accuracy4([RES1.det_acc_per_epoch[-1],RES2.det_acc_per_epoch[-1],RES3.det_acc_per_epoch[-1],RES4.det_acc_per_epoch[-1]],['Real - 1 net','Real  - mean of 3 nets','Real  - mean of 4 nets','Real - mean of 6 nets'], RES1.labels_names,labels_to_show,'Res: $2.2$,  Train:EMDs 8194,3295,8762, Test: $\\beta$-galactosidase,EMD-2984. ',RES_FOLDER+'r6_22_acc.png')
