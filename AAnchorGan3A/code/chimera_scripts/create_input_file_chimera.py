...
def create_candidates_file(source_file,target_file,thr_sigma=0.0):
    ...
    dbc.create_unlabeled_db(source_file,target_file,map_thr_sigma = thr_sigma)

if __name__ == "chimeraOpenSandbox":
    print "START"
    inp_file = sys.argv[3]
    out_file = sys.argv[4]
    thr_sigma = np.float(sys.argv[5])
    create_candidates_file(inp_file,out_file,thr_sigma = thr_sigma)
    runCommand('stop')
