...
class VAE_GAN1():
    def __init__(self):
        ...
        self.Loss_Mean_Sigma = alpha_mean*tf.losses.mean_squared_error(self.mp_fake_mean, dbloader.MEAN) + alpha_sigma*tf.losses.mean_squared_error(self.mp_fake_sigma, dbloader.SIGMA)
...
def Generate( z_var, reuse=False):
    ...
    with tf.variable_scope('generator') as scope:
        ...
        print("Debug Generate Shape Input",z_var.shape)
        d2 = tf.nn.elu(batch_normal(de_conv3(z_var , output_shape=[BATCH_SIZE, MAP_BOX_SIZE-6, MAP_BOX_SIZE-6, MAP_BOX_SIZE-6, 125],  k_dwh=[2,2,2],  d_dwh=[1,1,1],pad='SAME', name='gen_deconv2'), scope='gen_bn2', reuse=reuse))
        print("Debug Generate Shape L1",d2.shape)
        d3 = tf.nn.elu(batch_normal(de_conv3(d2, output_shape=[BATCH_SIZE, MAP_BOX_SIZE-4, MAP_BOX_SIZE-4, MAP_BOX_SIZE-4,126],  k_dwh=[3,3,3],  d_dwh=[1,1,1],pad = 'VALID',name='gen_deconv3'), scope='gen_bn3', reuse=reuse))
        print("Debug Generate Shape L3",d3.shape)
        d4 = tf.nn.elu(batch_normal(de_conv3(d3, output_shape=[BATCH_SIZE, MAP_BOX_SIZE-2, MAP_BOX_SIZE-2, MAP_BOX_SIZE-2,127], k_dwh=[3,3,3],  d_dwh=[1,1,1],pad = 'VALID', name='gen_deconv4'), scope='gen_bn4', reuse=reuse))
        print("Debug Generate Shape L4",d4.shape)
        d5 = de_conv3(d4, output_shape=[BATCH_SIZE, MAP_BOX_SIZE, MAP_BOX_SIZE, MAP_BOX_SIZE,1],  k_dwh=[3,3,3], pad = 'VALID', d_dwh=[1,1,1],name='gen_deconv5')
        print("Debug Generate Shape L5",d5.shape)
...
def Encode(x):
    ...
    with tf.variable_scope('encode', reuse = tf.AUTO_REUSE) as scope:
        ...
        print("DEBUG ENcode Shape - Input", x.shape)

        conv1 = tf.nn.elu(batch_normal(conv3d(x, output_dim=32, k_dwh=[5, 5, 5],  d_dwh=(1, 1, 1),  pad='VALID', name='e_c1'), scope='e_bn1'))
        print("DEBUG ENcode Shape - Out 1", conv1.shape)
        conv2 = tf.nn.elu(batch_normal(conv3d(conv1, output_dim=128, k_dwh=[4, 4, 4],  d_dwh=(1, 1, 1), pad='VALID',name='e_c2'), scope='e_bn2'))
        print("DEBUG ENcode Shape - Out 2", conv2.shape)
        conv3_1 = tf.nn.elu(batch_normal(conv3d(conv2 , output_dim=128,k_dwh=[4, 4, 4],  d_dwh=(1, 1, 1), pad='VALID', name='e_c31'), scope='e_bn31'))
        print("DEBUG ENcode Shape - Out 3_1", conv3_1.shape)
        conv3_2 = tf.nn.elu(batch_normal(conv3d(conv2 , output_dim=128,k_dwh=[4, 4, 4],  d_dwh=(1, 1, 1), pad='VALID', name='e_c32'), scope='e_bn32'))
        print("DEBUG ENcode Shape - Out 3_2", conv3_2.shape)
...
def discriminate( mp, vx, reuse=False):
    ...
    with tf.variable_scope("discriminator") as scope:
        ...
        if reuse:
            scope.reuse_variables()
        print("DISC Generate Shape Input",x_var.shape)
        conv1 = tf.nn.elu(conv3d(x_var, output_dim=32, k_dwh=[5, 5, 5],  d_dwh=(1, 1, 1), pad='VALID', name='dis_conv1'))
        print("DISC Generate Shape L1",conv1.shape)
        conv2= tf.nn.elu(batch_normal(conv3d(conv1, output_dim=32,k_dwh=[4, 4, 4],  d_dwh=(1, 1, 1), pad='VALID', name='dis_conv2'), scope='dis_bn1', reuse=reuse))
        print("DISC Generate Shape L2",conv2.shape)
        conv3= tf.nn.elu(batch_normal(conv3d(conv2, output_dim=32,k_dwh=[4, 4, 4],  d_dwh=(1, 1, 1), pad='VALID', name='dis_conv3'), scope='dis_bn2', reuse=reuse))
        print("DISC Generate Shape L3",conv3.shape)
        conv3= tf.reshape(conv3, [BATCH_SIZE, -1])
        fl = tf.nn.elu(batch_normal(fully_connect(conv3, output_size=32, scope='dis_fully1'), scope='dis_bn4', reuse=reuse))
        print("DISC Generate Shape L5",fl.shape)
        output = fully_connect(fl , output_size=1, scope='dis_fully2')
        print("DISC Generate Shape OUT",output.shape)
