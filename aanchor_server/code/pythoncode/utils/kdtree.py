...
class KDTree4(object):
    def __init__(self, points):
        eps = 10.**(-100)
        #
        self.eps = eps
        self.points4 = [np.append(points[k],k*eps) for k in range (len(points))]
        self.min_dist = len(points)*eps
        self.kdt = create(self.points4)
    def in_range(self,point,dist):
        dist = dist+self.min_dist
